	...
	while (offset < prog_size) {
		...
		switch ((unsigned char)program[offset]) {
			case 0x01:
				byte_buffer[b_count++] = program[offset++];
				text << "ADD  ";
				disasm_mod_r(program[offset], reg, rm, rex,
				             reg_size);
				byte_buffer[b_count++] = program[offset++];
				text << rm << ", " << reg;
				break;
			...
			case 0x0f:
				byte_buffer[b_count++] = program[offset++];
				disasm_0f(program, offset, byte_buffer, b_count,
				          text, rex, reg_size);
				break;
			case 0x29:
				byte_buffer[b_count++] = program[offset++];
			...
			case 0xbf:
				...
				if (rex & 0x08)
					text << reg_to_str(byte, 64) << ", "
					     << disasm_imm64(program, offset,
					                     byte_buffer,
					                     b_count);
				else
					text << reg_to_str(byte, 32) << ", "
					     << disasm_imm32(program, offset,
					                     byte_buffer,
					                     b_count);
				break;
			case 0xc3:
			...
			case 0xf6:
				byte_buffer[b_count++] = program[offset++];
				disasm_f6(program, offset, byte_buffer, b_count,
				          text, rex, reg_size);
				break;
			case 0xf7:
				byte_buffer[b_count++] = program[offset++];
				disasm_f7(program, offset, byte_buffer, b_count,
				          text, rex, reg_size);
				break;
			case 0xff:
				byte_buffer[b_count++] = program[offset++];
				disasm_ff(program, offset, byte_buffer, b_count,
				          text, rex, reg_size);
				break;
			default:
				byte_buffer[b_count++] = program[offset++];
...
static void disasm_0f(unsigned char *program, int &offset,
	...
	switch (byte) {
		case 0x84:
			...
			text << "JE ";
			if (reg_size == 32)
				text << disasm_imm32(program, offset,
				                     byte_buffer, b_count);
			else
				text << disasm_imm16(program, offset,
				                     byte_buffer, b_count);
			break;
		case 0x85:
			byte_buffer[b_count++] = program[offset++];
...
static void disasm_f6(unsigned char *program, int &offset,
	...
	switch (ireg) {
		case 0:
			text << "TEST " << rm << ", "
			     << disasm_imm8(program, offset, byte_buffer,
			                    b_count);
			break;
		default:
			text << "unknown f6";
